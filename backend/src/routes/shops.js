...
router.post('/', verifyToken, requireRole('Admin'), async (req, res) => {
    ...
    try {
        ...
        logActivity(req, 'create', 'shop', rows[0].id, `Added shop "${name}"`);
...
router.put('/:id', verifyToken, requireRole('Admin'), async (req, res) => {
    ...
    try {
        ...
        logActivity(req, 'update', 'shop', rows[0].id, `Updated shop "${name}"`);
        res.json(rows[0]);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Internal server error' });
...
router.delete('/:id', verifyToken, requireRole('Admin'), async (req, res) => {
    try {
        const { rows: existing } = await pool.query('SELECT name FROM shops WHERE id = $1', [req.params.id]);
        ...
        logActivity(req, 'delete', 'shop', req.params.id, `Deleted shop "${existing[0]?.name || req.params.id}"`);
